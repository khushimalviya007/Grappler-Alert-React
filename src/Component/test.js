...
const options3 = [
  { value: 'Notifiation', label: 'Notification' },
  { value: 'Alert', label: 'Alert' },
];
function Rules() {
  const nevigate = useNavigate();
  const [recepientDescription, setRecepientDescription] = useState([
    { value: 'Assigne To', label: 'Assigne To' },
    { value: 'Assigned By', label: 'Assigned By' },
  ...
  //     filteredOptions = selectedOptions.filter(option => option.value !== 'Assigne To' && option.value !== 'Assigned By');
  //   } else {
  //     // If 'Both' is not selected, remove 'Both' from the selection
  //     filteredOptions = selectedOptions.filter(option => option.value !== 'Both');
  //   }

  //   setSelectedValues1(filteredOptions);
  // };
  const handleRecipientChange = (selectedOptions) => {
    const bothSelected = selectedOptions.some(option => option.value === 'Both');
